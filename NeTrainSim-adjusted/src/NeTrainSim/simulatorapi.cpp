...
std::unique_ptr<SimulatorAPI>
    SimulatorAPI::instance(new SimulatorAPI());
...
{
    // Register enums
    qRegisterMetaType<SimulatorAPI::Mode>(
        "SimulatorAPI::Mode");

    // Register Train and its components
    qRegisterMetaType<Train>("Train");
    qRegisterMetaType<Train *>("Train*");
    qRegisterMetaType<std::shared_ptr<Train>>(
        "std::shared_ptr<Train>");
    qRegisterMetaType<std::string>("std::string");
    qRegisterMetaType<Vector<double>>("Vector<double>");
    qRegisterMetaType<Vector<std::shared_ptr<NetNode>>>(
        "Vector<std::shared_ptr<NetNode>>");
    qRegisterMetaType<Vector<std::shared_ptr<NetLink>>>(
        "Vector<std::shared_ptr<NetLink>>");
    qRegisterMetaType<Vector<std::shared_ptr<Car>>>(
        "Vector<std::shared_ptr<Car>>");
    qRegisterMetaType<Vector<std::shared_ptr<Locomotive>>>(
        "Vector<std::shared_ptr<Locomotive>>");
    qRegisterMetaType<
        Vector<Map<std::string, std::string>>>(
        "Vector<Map<std::string, std::string>>");
    qRegisterMetaType<Map<std::string, double>>(
        "Map<std::string, double>");
    qRegisterMetaType<std::map<std::string, std::any>>(
    ...
    qRegisterMetaType<Vector<std::shared_ptr<NetLink>>>(
        "Vector<std::shared_ptr<NetLink>>");
    qRegisterMetaType<Vector<std::shared_ptr<NetSignal>>>(
        "Vector<std::shared_ptr<NetSignal>>");
    qRegisterMetaType<std::pair<double, double>>(
        "std::pair<double, double>");
    qRegisterMetaType<std::shared_ptr<NetNode>>(
        "std::shared_ptr<NetNode>");
    qRegisterMetaType<std::shared_ptr<NetLink>>(
        "std::shared_ptr<NetLink>");
    qRegisterMetaType<std::shared_ptr<NetSignal>>(
...
void SimulatorAPI::resetInstance()
{
    if (instance)
    {
        // Get all network names from the thread-safe data
        // map
        QList<QString> networkNames =
            instance->apiDataMap.getNetworkNames();
        for (const QString &networkName : networkNames)
        {
            // Retrieve the APIData for the current network
            APIData data =
                instance->apiDataMap.get(networkName);
            ...
            {
                if (data.simulatorWorker->thread()
                    == data.workerThread)
                {
            ...
            if (data.workerThread)
            {
                ...
                delete data
                    .workerThread; // Delete the thread
            ...
            instance->apiDataMap.addOrUpdate(networkName,
                                             data);
        }
...
void SimulatorAPI::setLocale()
{
    ...
    {
        qWarning() << "Failed to set std::locale: "
                   << e.what();
    ...
    // Configure Qt's QLocale for UTF-8 without thousands
    // separators
    QLocale qtLocale(QLocale::English,
                     QLocale::UnitedStates);
    qtLocale.setNumberOptions(
        QLocale::OmitGroupSeparator); // Disable thousands
                                      // separator
...
void SimulatorAPI::createNewSimulationEnvironment(
    ...
    QJsonObject linksFileContent, QString networkName,
    QVector<QMap<QString, std::any>> trainsData,
    double timeStep, Mode mode)
{
    // Set locale to US format (no thousands separator, dot
    // for decimals)
    setLocale();
    ...
    auto nodeRec = ReadWriteNetwork::readNodesFromJson(
        nodesFileContent);
    auto linkRec = ReadWriteNetwork::readLinksFromJson(
        linksFileContent);

    auto nodeRecordsf =
        Utils::convertToQVectorString(nodeRec);
    auto linkRecordsf =
        Utils::convertToQVectorString(linkRec);
...
void SimulatorAPI::createNewSimulationEnvironmentFromFiles(
    ...
    double timeStep, Mode mode)
{
    ...
    auto nodeRecords = ReadWriteNetwork::readNodesFile(
        nodesFile.toStdString());

    auto linkRecords = ReadWriteNetwork::readLinksFile(
        linksFile.toStdString());

    auto trainsRecords = TrainsList::readTrainsFile(
        trainsFile.toStdString());

    auto nodeRecordsf =
        Utils::convertToQVectorString(nodeRecords);
    auto linkRecordsf =
        Utils::convertToQVectorString(linkRecords);

    auto trainsRecordsf =
        Utils::convertToQVector(trainsRecords);
    ...
    double timeStep, Mode mode)
{
    ...
    CHECK_TRUE(QObject::connect(
        simulatorWorker, &SimulatorWorker::simulatorLoaded,
        [this, localNetworkName, &loop,
         &setupSuccess](APIData &loadedData) {
            qInfo() << "Simulator loaded for network:"
                    << localNetworkName;
            apiDataMap.addOrUpdate(localNetworkName,
                                   loadedData);
    ...
    CHECK_TRUE(QObject::connect(
        simulatorWorker, &SimulatorWorker::errorOccured,
        [this, localNetworkName, workerThread, &loop,
         &setupSuccess](const QString &error) {
    ...
    // Invoke setup in worker thread with correct
    // networkName
    QMetaObject::invokeMethod(simulatorWorker, [=, this]() {
        APIData workerData =
            apiDataMap.get(localNetworkName);
        simulatorWorker->setupSimulator(
            workerData, localNetworkName, nodeRecords,
            linkRecords, trainList, timeStep);
    });

    loop.exec();

    // Only continue with connections if setup was
    // successful
    if (setupSuccess)
    {
        ...
        CHECK_TRUE(connect(
            workerThread, &QThread::finished, this,
            [this, localNetworkName]() {
                handleWorkersReady(localNetworkName);
            },
            Qt::DirectConnection));

        APIData latestApiData =
            apiDataMap.get(localNetworkName);
        if (latestApiData.workerThread)
        {
            latestApiData.workerThread->setPriority(
                QThread::LowPriority);
            apiDataMap.addOrUpdate(localNetworkName,
                                   latestApiData);
        }

        qInfo() << "Simulator setup complete for"
                << localNetworkName;
    }
    else
    {
        ...
        qWarning() << "Failed to set up simulator for"
                   << localNetworkName;
    }
}

void SimulatorAPI::setupConnections(
    const QString &networkName, Mode mode)
{
    ...
    {
        emit errorOccurred(
            QString("Simulator instance is nullptr! "
    ...
    CHECK_TRUE(connect(
        simulator, &Simulator::allTrainsReachedDestination,
        this, [this, networkName]() {
            handleAllTrainsReachedDestination(networkName);
        }));

    CHECK_TRUE(connect(
        simulator, &Simulator::resultDataAvailable, this,
    ...
    CHECK_TRUE(connect(
        simulator, &Simulator::simulationFinished, this,
        [this, networkName]() {
            qDebug() << "Current thread 3:"
    ...
    CHECK_TRUE(connect(
        ...
        [this, networkName,
         mode](double currentSimulatorTime,
               double progressPercent) {
            handleOneTimeStepCompleted(
                networkName, currentSimulatorTime,
    ...
    CHECK_TRUE(connect(
        simulator, &Simulator::progressUpdated, this,
        [this, networkName](double simulationTime,
    ...
    CHECK_TRUE(connect(
        simulator, &Simulator::simulationPaused, this,
        [this, networkName, mode]() {
            ...
            checkAndEmitSignal(
                ...
                &SimulatorAPI::simulationsPaused, mode);
        },
        mConnectionType));

    // Connect simulation resumed signal
    CHECK_TRUE(connect(
        simulator, &Simulator::simulationResumed, this,
        [this, networkName, mode]() {
            ...
            checkAndEmitSignal(
                ...
                mResumeTracker.getRequestedNetworks()
                    .size(),
                mResumeTracker.getRequestedNetworks(),
                &SimulatorAPI::simulationsResumed, mode);
        },
        mConnectionType));

    // Connect simulation terminated signal
    CHECK_TRUE(connect(
        simulator, &::Simulator::simulationTerminated, this,
        [this, networkName, mode]() {
            ...
            checkAndEmitSignal(
                ...
                mTerminateTracker.getRequestedNetworks()
                    .size(),
                mTerminateTracker.getRequestedNetworks(),
                &SimulatorAPI::simulationsTerminated, mode);
        },
        mConnectionType));

    // Connect simulation restarted signal
    CHECK_TRUE(connect(
        simulator, &Simulator::simulationRestarted, this,
        [this, networkName, mode]() {
            ...
            checkAndEmitSignal(
                ...
                mRestartTracker.getRequestedNetworks()
                    .size(),
                mRestartTracker.getRequestedNetworks(),
                &SimulatorAPI::simulationsRestarted, mode);
        },
        mConnectionType));

    CHECK_TRUE(connect(
        simulator, &Simulator::plotTrainsUpdated, this,
        [this, networkName,
         mode](Vector<
               std::pair<std::string,
                         Vector<std::pair<double, double>>>>
                   trainsStartEndPoints) {
            handleTrainCoordsUpdate(
                networkName, trainsStartEndPoints, mode);
        },
        Qt::QueuedConnection));

    CHECK_TRUE(connect(
        simulator, &Simulator::errorOccurred, this,
        [this, networkName](QString error) {
            emit errorOccurred(QString("Error in Network ")
                               + networkName + QString(": ")
                               + error);
        },
        Qt::QueuedConnection));
    ...
    // Update the APIData in the thread-safe map (if any
    // changes were made)
    ...
    QString networkName, Mode mode)
{
    ...
    {
        CHECK_TRUE(connect(
            train.get(), &Train::destinationReached, this,
            [this, networkName, train,
             mode](QJsonObject state) {
                handleTrainReachedDestination(networkName,
                                              state, mode);
            },
            Qt::QueuedConnection));
        CHECK_TRUE(connect(
            train.get(), &Train::containersLoaded, this,
            [this, networkName, train]() {
                emit containersAddedToTrain(
                    networkName, QString::fromStdString(
                                     train->trainUserID));
            },
            Qt::QueuedConnection));

        CHECK_TRUE(connect(
            train.get(), &Train::containersUnloaded, this,
            [this, train, networkName](
                QString trainID, QString seaPortCode,
                QJsonArray containers) {
                emit SimulatorAPI::ContainersUnloaded(
                    networkName, trainID, seaPortCode,
                    containers);
            },
            Qt::QueuedConnection));

        CHECK_TRUE(connect(
            train.get(), &Train::trainStateAvailable, this,
...
void SimulatorAPI::handleAllTrainsReachedDestination(
    QString networkName)
{
    emit allTrainsReachedDestination(networkName);
}

void SimulatorAPI::handleTrainReachedDestination(
    QString networkName, QJsonObject trainState, Mode mode)
{
...
void SimulatorAPI::handleResultsAvailable(
    QString networkName, TrainsResults result, Mode mode)
{
    // Emit single result immediately
    emit simulationResultsAvailable(networkName, result);
}

void SimulatorAPI::handleProgressUpdate(
    QString networkName, double simulationTime,
    int progressPercentage)
{
...
void SimulatorAPI::handleTrainCoordsUpdate(
    ...
    Mode mode)
{
    auto output = Utils::convertToQtTrainsCoords(
        simulatorTrainsCoords);
    ...
    double currentSimulatorProgress, Mode mode)
{
    ...
    mTimeStepTracker.addUpdateData(
        networkName,
        {currentSimulatorTime, currentSimulatorProgress});
    ...
    case Mode::Async: {
        ...
        const int completed =
            mTimeStepTracker.incrementAndGetCompleted();
        const int total =
            mTimeStepTracker.getRequestedCount();

        // Check if all simulators have completed their time
        // step
        if (completed == total)
        {
    ...
    default:
        ...
        emit errorOccurred("Unexpected mode in "
                           "handleOneTimeStepCompleted");
        qWarning() << "Unexpected mode in "
                      "handleOneTimeStepCompleted";
        break;
    }
}

void SimulatorAPI::handleSimulationFinished(
    QString networkName)
{
...
void SimulatorAPI::handleWorkersReady(QString networkName)
{
    ...
    case Mode::Async: {
        ...
        const int completed =
            mWorkerTracker.incrementAndGetCompleted();
        const int total =
            mWorkerTracker.getRequestedCount();
    ...
    default:
        emit errorOccurred(
            "Unexpected mode in handleWorkersReady");
        qWarning()
            << "Unexpected mode in handleWorkersReady";
        break;
    }
    ...
    QVector<QString> networkNames)
{
    ...
    {
        // Check if the network exists in the thread-safe
        // map
        if (!apiDataMap.contains(networkName))
        {
            emit errorOccurred("A network with name "
        ...
        if (apiData.simulator)
        {
            // Invoke the generateSummaryData method on the
            // simulator
            bool success = QMetaObject::invokeMethod(
                apiData.simulator, "generateSummaryData",
                mConnectionType);
            if (!success)
            {
                emit errorOccurred(
                    "Failed to invoke generateSummaryData");
                qWarning() << "Failed to invoke "
                              "generateSummaryData";
            }
        }
    }
...
void SimulatorAPI::requestPauseSimulation(
    QVector<QString> networkNames)
{
    // If "*" is specified, process all networks
    if (networkNames.contains("*"))
...
void SimulatorAPI::requestResumeSimulation(
    QVector<QString> networkNames)
{
    // If "*" is specified, process all networks
    if (networkNames.contains("*"))
...
void SimulatorAPI::requestTerminateSimulation(
    QVector<QString> networkNames)
{
...
void SimulatorAPI::addTrainToSimulation(
    QString                         networkName,
    QVector<std::shared_ptr<Train>> trains)
{
    // Check if the network exists in the thread-safe map
    ...
    for (auto &train : trains)
    {
        ...
        apiData.trains.insert(
            QString::fromStdString(train->trainUserID),
            train);

        // Invoke the addTrainToSimulation method on the
        // simulator
        bool success = QMetaObject::invokeMethod(
            apiData.simulator, "addTrainToSimulation",
            mConnectionType,
        ...
        {
            emit errorOccurred(
                "Failed to invoke addTraomToSimulation");
            qWarning()
                << "Failed to invoke addTrainToSimulation";
        }
...
std::shared_ptr<Train>
SimulatorAPI::getTrainByID(QString  networkName,
                           QString &trainID)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
...
SimulatorAPI::getAllTrains(QString networkName)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
    {
                                        ...
                                        QJsonObject json)
{
    ...
    if (subjectTrain)
    {
        result = subjectTrain->addContainers(json);
    }
    else
    {
        emit errorOccurred("Train with ID: " + trainID
                           + " does not exist");
    }
    return result;
}
...
void SimulatorAPI::requestRunSimulation(
    ...
    bool endSimulationAfterRun, bool getStepEndSignal)
{
    // If "*" is specified, process all networks
    if (networkNames.contains("*"))
    {
    ...
    for (const auto &networkName : networkNames)
    {
        ...
        mReachedDesTracker.setRequestedNetworks(
            networkNames);
        ...
        if (apiData.simulator)
        {
            ...
            // Invoke the runSimulation method on the
            // simulator
            bool success = QMetaObject::invokeMethod(
                apiData.simulator, "runSimulation",
                mConnectionType, Q_ARG(double, timeSteps),
            ...
            {
                qWarning()
                    << "Failed to invoke runSimulation";
            }
        }
    }
...
void SimulatorAPI::requestFinalizeSimulation(
    QVector<QString> networkNames)
{
    // If "*" is specified, process all networks
    if (networkNames.contains("*"))
    ...
    for (const auto &networkName : networkNames)
    {
        ...
        if (apiData.simulator)
        {
            // Invoke the finalizeSimulation method on the
            // simulator
            bool success = QMetaObject::invokeMethod(
    ...
    Mode mode)
{
    ...
    {
    case Mode::Async:
        // In async mode, emit signal when counter reaches
        // total
        if (counter == total)
        {
    ...
    default:
        // Handle unexpected mode
        qWarning()
            << "Unexpected mode in checkAndEmitSignal";
        break;
    }
}
        ...
        double timeStep, Mode mode)
{
    ...
    getInstance().createNewSimulationEnvironment(
        nodesFileContent, linksFileContent, networkName,
        trainList, timeStep, mode);
}
        ...
        double timeStep, Mode mode)
{
    ...
    getInstance().createNewSimulationEnvironment(
        networkName, nodeRecords, linkRecords, trainList,
        timeStep, mode);
}
        ...
        double timeStep, Mode mode)
{
    ...
    getInstance().createNewSimulationEnvironmentFromFiles(
        nodesFile, linksFile, networkName, trainsFile,
        timeStep, mode);
}
...
void SimulatorAPI::InteractiveMode::runSimulation(
    QVector<QString> networkNames, double timeSteps,
    bool getProgressSignal)
{
    ...
    getInstance().requestRunSimulation(
        networkNames, timeSteps, endSimulationAfterRun,
        getProgressSignal);
}
...
void SimulatorAPI::InteractiveMode::terminateSimulation(
    QVector<QString> networkNames)
{
...
Network *SimulatorAPI::InteractiveMode::getNetwork(
    QString networkName)
{
...
QVector<std::shared_ptr<Train>>
SimulatorAPI::InteractiveMode::getAllTrains(
    QString networkName)
{
...
Network *SimulatorAPI::ContinuousMode::getNetwork(
    QString networkName)
{
    return getInstance().getNetwork(networkName);
}
