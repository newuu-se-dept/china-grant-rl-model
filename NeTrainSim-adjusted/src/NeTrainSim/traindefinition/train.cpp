...
Train::Train(
    int simulatorID, string id, Vector<int> trainPath,
    ...
    : QObject(nullptr)
{
    ...
    this->rearrangeTrain(); // rearrange order of each loco
                            // and car
    ...
    this->betweenNodesLengths = Vector<Vector<double>>(
        sizeOfPath, Vector<double>(sizeOfPath));
    this->LowerSpeedNodeIDs =
        Vector<Vector<Map<int, double>>>(
            sizeOfPath,
            Vector<Map<int, double>>(sizeOfPath,
                                     Map<int, double>()));
    ...
    setOptimization(optimize, optimization_k,
                    runOptimizationEvery,
                    optimizationLookaheadSteps);
};
    ...
    int    optimizationLookaheadSteps)
{
    ...
    // populating it
    if (throttleLevels.size() > 0)
    {
        throttleLevels.clear();
    }
    for (int n = 0; n <= this->locomotives.front()->Nmax;
         n++)
    {
        this->throttleLevels.push_back(std::pow(
            (((double)n) / this->locomotives.front()->Nmax),
            2.0));
    }
    this->lookAheadCounterToUpdate = runOptimizationEvery;
    this->mem_lookAheadCounterToUpdate =
        runOptimizationEvery;
    this->lookAheadStepCounter = optimizationLookaheadSteps;
    this->mem_lookAheadStepCounter =
        optimizationLookaheadSteps;
}
...
{
    this->ActiveLocos.erase(
        std::remove_if(
            this->ActiveLocos.begin(),
    ...
    if (this->ActiveLocos.size() == 0)
    {
        ...
        // ErrorHandler::showNotification("All locomotives
        // of train (" + std::to_string(this->id) + ") are
        // out of energy");
    }
...
double Train::getBatteryNetEnergyConsumed()
{
    ...
    {
        total +=
            vehicle->getBatteryCumNetEnergyConsumption();
    }
    return total;
}
...
double Train::getAverageTendersTankStatus()
{
    ...
    for (auto &car : this->cars)
    {
        if (TrainTypes::carNonRechargableTechnologies.exist(
                car->carType))
        {
...
double Train::getAverageTendersBatteryStatus()
{
    ...
    for (auto &car : this->cars)
    {
        if (TrainTypes::carRechargableTechnologies.exist(
                car->carType))
        {
...
{
    return this->getCargoNetWeight()
           * (this->travelledDistance / (double)1000.0);
}
...
Map<std::string, double> Train::getTrainConsumedTank()
{
    ...
    for (auto &loco : this->locomotives)
    {
        ...
        if (TrainTypes::getFuelTypeFromPowerType(
                loco->powerType)
            == TrainTypes::FuelType::noFuel)
        {
            continue;
        }
        // add the consumed fuel amount
        if (consumption.is_key(TrainTypes::fuelTypeToStr(
                TrainTypes::getFuelTypeFromPowerType(
                    loco->powerType))))
        {
            consumption[TrainTypes::fuelTypeToStr(
                TrainTypes::getFuelTypeFromPowerType(
                    loco->powerType))] +=
                loco->getTankCumConsumedFuel();
        }
        else
        {
            consumption[TrainTypes::fuelTypeToStr(
...
//     this->setTrainWeight();
// }
// void Train::setLocomotives(Vector<Locomotive>
// locomotives) {
//     this->locomotives = locomotives;
//     this->nlocs = static_cast<int>(locomotives.size());
//     this->setTrainLength();
...
Map<std::shared_ptr<TrainComponent>, double>
Train::getTrainCentroids()
{
    ...
    for (auto &vehicle : this->trainVehicles)
    {
        ...
        mapper.emplace_hint(mapper.end(), std::ref(vehicle),
                            cumLength);
    }
...
void Train::rearrangeTrain()
{
    ...
    if (this->nlocs == 1 || this->nCars == 0)
    {
        for (auto &loco : this->locomotives)
        {
            this->trainVehicles.push_back(loco);
        }
        if (this->cars.size() > 0)
        {
            for (auto &car : this->cars)
            {
                this->trainVehicles.push_back(car);
            }
        }
    }
    else if ((this->nlocs > 1 && this->nlocs < 7)
             || (this->nCars < 2))
    {
        int locoSize      = this->nlocs / 2;
        int firstLocoSize = this->nlocs - locoSize;
        for (int i = 0; i < firstLocoSize; i++)
        {
            this->trainVehicles.push_back(
                (this->locomotives.at(i)));
        }
        if (this->cars.size() > 0)
        {
        ...
        for (int i = firstLocoSize;
             i < this->locomotives.size(); i++)
        {
            this->trainVehicles.push_back(
                (this->locomotives.at(i)));
        }
    ...
    else
    {
        ...
        // append second locos
        for (int i = firstLocoSize;
             i < (firstLocoSize + locoSize); i++)
        {
            this->trainVehicles.push_back(
                (this->locomotives.at(i)));
        }
        // append second cars
        for (int i = firstCarSize;
             i < firstCarSize + carSize; i++)
        {
            this->trainVehicles.push_back(
                (this->cars.at(i)));
        }
        // append third locos
        for (int i = (firstLocoSize + locoSize);
             i < this->locomotives.size(); i++)
        {
            this->trainVehicles.push_back(
                (this->locomotives.at(i)));
...
void Train::setTrainLength()
{
    ...
    for (Vector<std::shared_ptr<TrainComponent>>::iterator
             it = this->trainVehicles.begin();
         it != this->trainVehicles.end(); ++it)
    {
...
void Train::setTrainWeight()
{
    ...
    for (Vector<std::shared_ptr<Locomotive>>::iterator it =
             this->locomotives.begin();
         it != this->locomotives.end(); ++it)
    {
        totalTrainWeight += it->get()->currentWeight;
    }

    // loop over all cars
    for (Vector<std::shared_ptr<Car>>::iterator it =
             this->cars.begin();
         it != this->cars.end(); ++it)
    {
        totalTrainWeight += it->get()->currentWeight;
    ...
    this->totalMass =
        totalTrainWeight * 1000; // convert to kg
    this->totalEmptyMass =
        totalEmptyWeight * 1000; // convert to kg
};

void Train::updateGradesCurvatures(
    const Vector<double> &trainGrades,
    const Vector<double> &trainCurvature)
{
    ...
    {
        this->trainVehicles[i]->trackCurvature =
            trainCurvature[i];
        this->trainVehicles[i]->trackGrade = trainGrades[i];
    }
}

Vector<std::shared_ptr<Car>>
Train::getActiveTanksOfType(TrainTypes::CarType cartype)
{
    ...
    if (!this->carsTypes[cartype].empty())
    {
        ...
        {
            if (car->getBatteryStateOfCharge() > 0.0
                || car->getTankCurrentCapacity() > 0.0)
            {
...
void Train::updateLocNotch()
{
    ...
    {
        for (Vector<std::shared_ptr<Locomotive>>::iterator
                 it = this->locomotives.begin();
             it != this->locomotives.end(); ++it)
        {
...
double Train::getTotalTractiveForce(
    double speed, double acceleration, bool optimize,
    double optimumThrottleLevel)
{
    ...
    for (Vector<std::shared_ptr<Locomotive>>::iterator it =
         ...
         it != this->locomotives.end(); ++it)
    {
        it->get()->currentTractiveForce =
            it->get()->getTractiveForce(
                this->coefficientOfFriction, speed,
...
double Train::getAccelerationUpperBound(
    double speed, double acceleration, double freeFlowSpeed,
    bool optimize, double optimumThrottleLevel)
{
    double t = 0.0, r = 0.0, a_max = 0.0;
    t     = this->getTotalTractiveForce(speed, acceleration,
                                        optimize,
...
double Train::getSafeGap(double initialGap, double speed,
                         double freeFlowSpeed, double T_s,
                         bool estimate)
{
    double gap_lad = 0;
    if (!estimate)
                                   ...
                                   double deltaT)
{
    ...
    if (u_hat < speed)
    {
        u_hat = max(
            u_hat,
            speed - this->coefficientOfFriction * deltaT);
    }
...
double Train::getTimeToCollision(double gap, double minGap,
                                 double speed,
                                 double leaderSpeed)
{
    return min((gap - minGap)
                   / max(speed - leaderSpeed, 0.0001),
               100.0);
}

double Train::get_acceleration_an11(double u_hat,
                                    double speed,
                                    double TTC_s,
                                    double frictionCoef)
{
    ...
    return max(((u_hat - speed) / (denominator)),
               -frictionCoef * this->g);
}

/**
 * @brief   Calculates the acceleration of a train to reach
 * a predicted speed based on its current speed, maximum
 * acceleration, and time constant.
 *
 ...
 * @param amax  The maximum acceleration in meters per
 * second squared (m/s^2).
 * @return The acceleration in meters per second squared
 * (m/s^2).
 */
double Train::get_acceleration_an12(double u_hat,
                                    double speed,
                                    double T_s, double amax)
{
...
double Train::get_acceleration_an13(double beta1,
                                    double an11,
                                    double an12)
{
...
double Train::get_acceleration_an14(double speed,
                                    double leaderSpeed,
                                    double T_s, double amax,
                                    double frictionCoef)
{
    return max(min((leaderSpeed - speed) / T_s, amax),
               -frictionCoef * this->g);
}
...
double Train::get_acceleration_an1(double beta2,
                                   double an13, double an14)
{
...
double Train::get_acceleration_an2(
...
double Train::accelerate(double gap, double mingap,
                         double speed, double acceleration,
                         double leaderSpeed,
                         double freeFlowSpeed,
                         double deltaT, bool optimize,
                         double throttleLevel)
{
    ...
    double amax = this->getAccelerationUpperBound(
        speed, acceleration, freeFlowSpeed, optimize,
        throttleLevel);
    if ((gap > this->getSafeGap(mingap, speed,
                                freeFlowSpeed, this->T_s,
                                false))
        && (amax > 0))
    {
    ...
    double u_hat = this->getNextTimeStepSpeed(
        gap, mingap, speed, freeFlowSpeed, amax, this->T_s,
        deltaT);
    double TTC_s = this->getTimeToCollision(
        gap, mingap, speed, leaderSpeed);
    double an11 = this->get_acceleration_an11(
        u_hat, speed, TTC_s, this->coefficientOfFriction);
    double an12 = this->get_acceleration_an12(
        u_hat, speed, this->T_s, amax);
    double beta1 = this->get_beta1(an11);
    double an13 =
        this->get_acceleration_an13(beta1, an11, an12);
    double an14 = this->get_acceleration_an14(
        speed, leaderSpeed, this->T_s, amax,
    ...
    double an2   = this->get_acceleration_an2(
        gap, mingap, speed, leaderSpeed, this->T_s,
        this->coefficientOfFriction);
    double a = an1 * (1.0 - gamma) - gamma * an2;
    return a;
}

double Train::accelerateConsideringJerk(
    double acceleration, double previousAcceleration,
    double jerk, double deltaT)
{
    double an =
        min(abs(acceleration),
            abs(previousAcceleration) + jerk * deltaT);
    return an * ((acceleration > 0) ? 1 : -1);
}

double
Train::smoothAccelerate(double acceleration,
...
{
    double u_next =
        min(previousSpeed + acceleration * deltaT,
            freeFlowSpeed);
...
void Train::checkSuddenAccChange(
    double previousAcceleration, double currentAcceleration,
    double deltaT)
{
    if (std::abs(
...
double Train::getStepAcceleration(
    double timeStep, double freeFlowSpeed,
    Vector<double> &gapToNextCriticalPoint,
    Vector<bool>   &gapToNextCriticalPointType,
    Vector<double> &leaderSpeed)
{

    // decrease the given quota of the future number of
    // steps covered for virtual simulation to update
    if (this->optimize)
    {
        ...
        {
            optimumThrottleLevel =
                optimumThrottleLevels.front();
        }
        if (optimumThrottleLevels.size() > 1)
        {
            // remove the used throttle level from the
            // vector
            optimumThrottleLevels.erase(
                optimumThrottleLevels.begin());
        }
    }
    ...
    for (int i = 0; i < gapToNextCriticalPoint.size(); i++)
    {
        if (!(gapToNextCriticalPointType)[i])
        {
            allAccelerations.push_back(this->accelerate(
                (gapToNextCriticalPoint)[i], minGap,
                this->currentSpeed,
                this->currentAcceleration, (leaderSpeed)[i],
                freeFlowSpeed, timeStep, this->optimize));
        }
        else
        {
            allAccelerations.push_back(this->accelerate(
                (gapToNextCriticalPoint)[i], GapFollowing,
                this->currentSpeed,
                this->currentAcceleration, (leaderSpeed)[i],
                freeFlowSpeed, timeStep, this->optimize));
        }
    }
    // get the minimum acceleration from all the
    // accelerations
    ...
    {
        int newIndx =
            allAccelerations.index(nonsmoothedAcceleration);

        if (!(gapToNextCriticalPointType)[newIndx])
        {
            this->accelerate(
                (gapToNextCriticalPoint)[newIndx], minGap,
                this->currentSpeed,
                this->currentAcceleration,
                (leaderSpeed)[newIndx], freeFlowSpeed,
                timeStep, this->optimize);
        }
        else
        {
            this->accelerate(
                (gapToNextCriticalPoint)[newIndx],
                GapFollowing, this->currentSpeed,
                this->currentAcceleration,
                (leaderSpeed)[newIndx], freeFlowSpeed,
                timeStep, this->optimize);
        }
    ...
    if (nonsmoothedAcceleration < 0.0
        ...
        && gapToNextCriticalPoint.back() > 50)
    {
        if (this->NoPowerCountStep < 5)
        {
            ...
            message << "Train " << this->id
                    << " Resistance is "
                    << "larger than train tractive force "
                       "at distance "
                    << travelledDistance << "(m)!\n";
            emit slowSpeedOrStopped(message.str());
            Logger::Logger::logMessage(
                Logger::LogLevel::WARNING, message.str());
            NoPowerCountStep++;
        }
    }
    // smooth the acceleration and consider jerk
    double smoothedAcceleration = this->smoothAccelerate(
        nonsmoothedAcceleration, this->previousAcceleration,
        1.0);
    double jerkedAcceleration =
        this->accelerateConsideringJerk(
            smoothedAcceleration,
            this->previousAcceleration, this->maxJerk,
            timeStep);
    if (round(this->currentSpeed * 1000) / 1000 == 0.0
        && jerkedAcceleration < 0)
    {
...
void Train::moveTrain(
    double currentSimulationTime, double timeStep,
    double          freeFlowSpeed,
    Vector<double> &gapToNextCriticalPoint,
    Vector<bool>   &gapToNextCriticalPointType,
    Vector<double> &leaderSpeed)
{
    ...
    double jerkedAcceleration = this->getStepAcceleration(
        timeStep, freeFlowSpeed, gapToNextCriticalPoint,
        gapToNextCriticalPointType, leaderSpeed);
    this->currentAcceleration = jerkedAcceleration;
    this->previousSpeed       = this->currentSpeed;
    this->currentSpeed        = this->speedUpDown(
        this->previousSpeed, this->currentAcceleration,
        timeStep, freeFlowSpeed);
    this->currentAcceleration = this->adjustAcceleration(
        this->currentSpeed, this->previousSpeed, timeStep);
    this->checkSuddenAccChange(this->previousAcceleration,
                               this->currentAcceleration,
                               timeStep);
    this->travelledDistance +=
        this->currentSpeed * timeStep;

    // update the throttle level of the train
    this->updateLocNotch();

    if ((std::round(trainTotalPathLength * 1000.0) / 1000.0)
        <= (std::round(travelledDistance * 1000.0)
            / 1000.0))
    {
...
void Train::forceTrainToStopFor(double duration,
                                double currentTime)
{
...
double
Train::getRemainingDwellTime(double currentTime) const
{
    if (dwellStartTime < 0)
        return 0;
...
void Train::calcTrainStats(
    Vector<double> listOfLinksFreeFlowSpeeds,
    double MinFreeFlow, double timeStep,
    std::string currentRegion)
{
    std::pair<Vector<double>, double> pwr =
        this->getTractivePower(
    ...
    this->cumUsedTractivePower +=
        this->currentUsedTractivePower;

    this->isOn =
        this->consumeEnergy(timeStep, this->currentSpeed,
                            currentUsedTractivePowerList);
    this->calculateEnergyConsumption(timeStep,
                                     currentRegion);

    this->tripTime += timeStep;
    this->delayTimeStat =
        this->getDelayTimeStat(MinFreeFlow, timeStep);
    this->cumDelayTimeStat += this->delayTimeStat;
    this->maxDelayTimeStat = this->getMaxDelayTimeStat(
        listOfLinksFreeFlowSpeeds, timeStep);
    this->cumMaxDelayTimeStat += this->maxDelayTimeStat;
    this->stoppedStat = this->getStoppingTimeStat(
        listOfLinksFreeFlowSpeeds);
    this->cumStoppedStat += this->stoppedStat;
    this->averageSpeed = this->calculateAverage(
        this->averageSpeed, this->currentSpeed, timeStep);
    this->averageAcceleration = this->calculateAverage(
        this->currentAcceleration,
        this->currentAcceleration, timeStep);
}
...
double Train::getMaxDelayTimeStat(
    Vector<double> listOfLinksFreeFlowSpeeds,
    double         timeStep)
{
    ...
    {
        finalMaxDelay +=
            (1 - (this->currentSpeed / uf)) * timeStep;
    }
                               ...
                               double timeStep)
{
    ...
    finalDelay += (1 - (this->currentSpeed / freeflowSpeed))
                  * timeStep;
    return finalDelay;
}

double Train::getStoppingTimeStat(
    Vector<double> listOfLinksFreeFlowSpeeds)
{
    ...
    if (this->previousSpeed > this->currentSpeed)
    {
        ...
        return (finalStopping)
               / (this->nCars + this->nlocs);
    }
    else
    {
        return 0.0;
...
Train::getLoadedContainers() const
{
    QVector<ContainerCore::Container *> containerList;
    for (auto &container :
         mLoadedContainers.getAllContainers())
    {
    ...
    ContainerCore::Container *container)
{
    if (container)
    {
        container->setContainerCurrentLocation(
            "Train_"
            + QString::fromStdString(this->trainUserID));
        mLoadedContainers.addContainer(
            container->getContainerID(), container);
        emit containersLoaded();
    }
}

bool Train::addContainers(QJsonObject json)
{
    auto containers =
        ContainerCore::ContainerMap::loadContainersFromJson(
            json);
...
Train::getContainersLeavingAtPort(
...
Train::countContainersLeavingAtPort(
    const QVector<QString> &portNames)
{
    if (portNames.isEmpty())
    {
    ...
    for (const QString &portName : portNames)
    {
        count =
            mLoadedContainers
                .countContainersByNextDestination(portName);
        if (count != 0)
        {
...
void Train::requestUnloadContainersAtTerminal(
    const QVector<QString> &portNames)
{
    if (isCurrentlyDwelling() || reachedDestination)
    {
        ...
        emit containersUnloaded(
            QString::fromStdString(this->trainUserID),
            containers.first, containersJson);
    }
}
#endif
...
int Train::getRechargableLocsNumber()
{
    ...
    for (Vector<std::shared_ptr<Locomotive>>::iterator it =
         ...
         it != this->locomotives.end(); ++it)
    {
        if (TrainTypes::locomotiveRechargableTechnologies
                .exist(it->get()->powerType))
        {
            count++;
        }
    }
...
tuple<double, double, double> Train::AStarOptimization(
    double prevSpeed, double currentSpeed,
    double currentAcceleration, double prevThrottle,
    Vector<double> vector_grade,
    Vector<double> vector_curvature, double freeSpeed_ms,
    double timeStep, Vector<double> u_leader,
    Vector<double> gapToNextCriticalPoint)
{
    this->updateGradesCurvatures(vector_grade,
                                 vector_curvature);
    double resistance =
        this->getTotalResistance(currentSpeed);
    ...
    {
        // if the throttle level (and resultant force) is
        // less than resistance, then the train is not
        // moving forward then discard this throttle level
        if (resistance < this->getTotalTractiveForce(
                currentSpeed, currentAcceleration, true,
                throttleLevel)
            || throttleLevel == this->throttleLevels.back())
        {
            ...
            // if there is no gap fed to the function,
            // consider a finite gap. this is only in case
            // the train could not calculate the next step
            // gap.
            if (gapToNextCriticalPoint.size() > 0)
            {
                Vector<double> allAcc = Vector<double>();
                // loop through all the gaps to next
                // train/station
                for (int i = 0;
                     i < gapToNextCriticalPoint.size(); i++)
                {
                    // get all accelerations and append them
                    // to the acceleration vector
                    allAcc.push_back(this->accelerate(
                        gapToNextCriticalPoint[i], 0.0,
                        currentSpeed, currentAcceleration,
                        u_leader[i], freeSpeed_ms, timeStep,
                        true, throttleLevel));
                }
            ...
            {
                stepAcceleration = this->accelerate(
                    std::numeric_limits<double>::infinity(),
                    0.0, currentSpeed, currentAcceleration,
                    0.0, freeSpeed_ms, timeStep, true,
                    throttleLevel);
            }
            // get speed after acceleration, jerk is not
            // considered here, since it will be
            // automatically considered in the true
            // calculations of the acceleration
            double stepSpeed = this->speedUpDown(
                prevSpeed, stepAcceleration, timeStep,
                freeSpeed_ms);
            // get the energy for the train if that
            // particular throttle level is used till the
            // end of the look ahead
            double energy = this->heuristicFunction(
                gapToNextCriticalPoint.back(),
                stepAcceleration, stepSpeed, timeStep,
                resistance, currentSpeed, prevSpeed);

            // append the step values to their corresponding
            // vectors
    ...
    // if there is no throttle consider (wont happen),
    // return the previous recommendation
    if (energyVec.size() == 0)
    {
        return std::make_tuple(currentSpeed,
                               currentAcceleration,
                               prevThrottle);
    }
    ...
    // if both the min and max energy are equal, return the
    ...
    auto normalize = [](double value, double min_value,
                        double max_value) -> double {
        // return max_value != min_value ? (value -
        // min_value) / (max_value - min_value) : 0.5;
        if (max_value != min_value)
        {
            double normalized = (value - min_value)
                                / (max_value - min_value);
            return 0.1 + 0.9 * normalized; // Scale to range
                                           // [0.1, 1]
        }
        return 0.5;
    };
    ...
    std::transform(energyVec.begin(), energyVec.end(),
                   std::back_inserter(energyLN),
                   [&normalize, &max_energy,
                    &min_energy](double energy) {
                       return normalize(energy, min_energy,
                                        max_energy);
                   });

    std::transform(
        speedVec.begin(), speedVec.end(),
        std::back_inserter(speedLN),
    ...
    double weight_energy =
        (double)1.0 - optimizeForSpeedNormalizedWeight;
    double weight_speed = optimizeForSpeedNormalizedWeight;

    // calculate the weighted normalize energy consumption
    // (goal programming)
    Vector<double> weighted_sums;
    for (size_t i = 0; i < energyLN.size(); ++i)
    {
        double weighted_sum =
            weight_energy * energyLN[i]
            - weight_speed * std::log(speedLN[i] + 1e-6);
        weighted_sums.push_back(weighted_sum);
    }

    // get the minimum weighted value index
    int idx_optimum =
        weighted_sums.index(weighted_sums.min());

    // return the corresponding low combined energy
    // consumption data
    return std::make_tuple(speedVec[idx_optimum],
                           accelerationVec[idx_optimum],
                           throttleVec[idx_optimum]);

    //    // get the minimum heuristic energy and the
    //    corresponding throttle level int minI =
    //    energyVec.argmin();
    //    // return the values corresponding to the min
    //    energy for the next step analysis return
    //    std::make_tuple(speedVec[minI],
    //    accelerationVec[minI], throttleVec[minI]);
}

double Train::heuristicFunction(
    double distanceToEnd, double stepAcceleration,
    double stepSpeed, double timeStep, double resistance,
    double currentSpeed, double prevSpeed)
{
    if (distanceToEnd == 0)
    {
        distanceToEnd = this->lookAheadStepCounter
                        * timeStep * currentSpeed;
    }
    // predict time needed to travel the segment
    double timeInterval =
        distanceToEnd / max(stepSpeed, 0.0001);
    // get the tractive power to travel a step forward
    pair<Vector<double>, double> out =
        this->getTractivePower(stepSpeed, stepAcceleration,
                               resistance);
    // get the energy consumption given the timeInterval
    return this->getTotalEnergyConsumption(
        timeInterval, stepSpeed, stepAcceleration,
        out.first);
}

double Train::pickOptimalThrottleLevelAStar(
    Vector<double> throttleLevels,
    int            lookAheadCounterToUpdate)
{
    optimumThrottleLevels = throttleLevels;
    return 0.0;
    //    int end = ( lookAheadCounterToUpdate <=
    //    throttleLevels.size()) ? lookAheadCounterToUpdate
    //    : throttleLevels.size(); if (end == 0){
    //        return this->optimumThrottleLevel;
    //    }
    //    this->optimumThrottleLevel =
    //    *std::max_element(throttleLevels.begin(),
    //    throttleLevels.begin() + end); return
    //    this->optimumThrottleLevel;
}

pair<Vector<double>, double>
Train::getTractivePower(double speed, double acceleration,
                        double resistanceForces)
{
    ...
    {
        return {currentVirtualTractivePowerList,
                currentVirtualTractivePower};
    }
    else
    {
        ...
        // divide the weight and resistance equally to all
        // active locomotives
        double oneLocoWeight = this->totalMass / (double)n;
        double oneLocoResistance =
            resistanceForces / (double)n;
        ...
        if (!this->ActiveLocos.empty())
        {
            ...
            {
                virtualPower =
                    loco->getSharedVirtualTractivePower(
                        speed, acceleration, oneLocoWeight,
                        oneLocoResistance);
                currentVirtualTractivePowerList.push_back(
                    virtualPower);
            }
        }
        currentVirtualTractivePower =
            currentVirtualTractivePowerList.sum();
        return {currentVirtualTractivePowerList,
                currentVirtualTractivePower};
    }
}
...
{
    for (std::shared_ptr<TrainComponent> &vehicle :
         this->trainVehicles)
    {
...
double Train::getTotalEnergyConsumption(
    double &timeStep, double &trainSpeed,
    double &acceleration, Vector<double> &usedTractivePower)
{
    if (usedTractivePower.empty())
    {
        return 0.0;
    }
    double energy = 0.0;
    //    double averageSpeed = (this->currentSpeed +
    //    this->previousSpeed) / (double)2.0;
    for (int i = 0; i < this->ActiveLocos.size(); i++)
    {
        energy +=
            this->ActiveLocos.at(i)->getEnergyConsumption(
                usedTractivePower.at(i), acceleration,
                trainSpeed, timeStep);
    }
    return energy;
}
...
void Train::reducePower(double &reductionFactor)
{
    ...
    {
        loco->reducePower(
            reductionFactor); // TODO: make this only for
                              // the affected locomotives
    }
}
...
void Train::setTrainsCurrentLinks(
    Vector<std::shared_ptr<NetLink>> newLinks)
{
    // clear the vector
                          ...
                          Vector<double> &usedTractivePower)
{
    ...
    int offLocos =
        0; // count the number of locomotives that are off
    double EC_kwh = 0; // locomotive Energy consumption

    // if the train is not moving and it does not consume
    // power, skip.
    if (usedTractivePower.empty())
    {
        return true;
    ...
    {
        // for (auto& loco : this->ActiveLocos) {
        //  if the locomotive is on, compute the energy
        //  consumption
        if (this->ActiveLocos.at(i)->isLocOn)
        {
            ...
            EC_kwh               = this->ActiveLocos.at(i)
                         ->getEnergyConsumption(
                             UsedTractiveP,
                             this->currentAcceleration,
                             averageSpeed, timeStep);

            // consume/recharge fuel from/to the locomotive
            // if it still has fuel or can be rechargable
            auto out = this->ActiveLocos.at(i)->consumeFuel(
                timeStep, trainSpeed, EC_kwh,
                UsedTractiveP);
            // bool fuelConsumed = out.first;
            double restEC = out.second;

            // if it is energy consumption and fuel was not
            // consumed from the locomotive, consume it by
            // the tenders
            if (restEC > 0.0)
            {
                // consume it equally from the tenders with
                // similar fuel type
                std::pair<bool, double>
                    fuelConsumedFromTender =
                        this->consumeTendersEnergy(
                            timeStep, trainSpeed, restEC,
                            this->ActiveLocos.at(i)
                                ->powerType);
                if (!fuelConsumedFromTender.first
                    && restEC == EC_kwh)
                {
                    this->ActiveLocos.at(i)->isLocOn =
                        false;
                }
            }
            // if it is energy regenerated and fuel was not
            // recharged to the locomotive, recharge the
            // cars' batteries
            else if (restEC < 0.0)
            {
                // recharge cars batteries
                this->rechargeCarsBatteries(
                    timeStep, EC_kwh,
                    this->ActiveLocos.at(i));
            }
        }
...
std::pair<bool, double> Train::consumeTendersEnergy(
    double timeStep, double trainSpeed, double EC_kwh,
    ...
    double hydrogenConversionFactor, double dieselDensity)
{
    ...
    int    count =
        this->ActiveCarsTypes[TrainTypes::powerToCarMap.at(
                                  powerType)]
            .size();
    double ECD = 0.0;
    if (count > 0)
    ...
    for (auto &car :
         this->ActiveCarsTypes[TrainTypes::powerToCarMap.at(
             powerType)])
    {
        // if the tender/battery still has energy to draw
        // from, consume it
        if (car->getBatteryCurrentCharge() > 0
            || car->getTankCurrentCapacity() > 0)
        {
            std::pair<bool, double> out = car->consumeFuel(
                timeStep, trainSpeed, ECD, 0.0,
        ...
        {
            this->ActiveCarsTypes[TrainTypes::powerToCarMap
                                      .at(powerType)]
                .removeValue(car);
            notConsumed += ECD;
        }
    }
    return std::make_pair(!(EC_kwh == notConsumed),
                          notConsumed);
    // return consumed;
}

bool Train::rechargeCarsBatteries(
    double timeStep, double EC_kwh,
    std::shared_ptr<Locomotive> &loco)
{
    bool consumed = false;
    int  count =
        this->carsTypes[TrainTypes::CarType::batteryTender]
            .size();
    double ECD = 0.0;
    ...
    {
        return loco->rechargeCatenary(
            EC_kwh); // recharge the catenary if the train
                     // does not have tenders
    }
    // refill all cars by that shared portion
    for (auto &car :
         this->carsTypes
             [TrainTypes::CarType::batteryTender])
    {
    ...
    // if the train does not have tenders or the tenders are
    // full, feed to the catenary
    if (!consumed)
    {
...
void Train::calculateEnergyConsumption(
    double timeStep, std::string currentRegion)
{
    ...
    if (this->cumRegionalConsumedEnergyStat.count(
            currentRegion)
        > 0)
    {
        this->cumRegionalConsumedEnergyStat[currentRegion] =
            this->cumRegionalConsumedEnergyStat
                [currentRegion]
            + this->energyStat;
    }
    else
...
Map<TrainTypes::PowerType, double>
Train::getMaxProvidedEnergyFromLocomotivesOnly(
    double &timeStep)
{
    Map<TrainTypes::PowerType, double> trainAllEC;
    if (this->ActiveLocos.size() < 1)
    {
        return trainAllEC;
    }
    for (auto &loco : this->ActiveLocos)
    {
        double locoEC =
            loco->getMaxProvidedEnergy(timeStep);
        // add value to locoRestEC
        if (trainAllEC.is_key(loco->powerType))
        {
            trainAllEC[loco->powerType] = locoEC;
        }
        else
        {
            trainAllEC[loco->powerType] += locoEC;
...
Map<TrainTypes::PowerType, double>
Train::getMaxProvidedEnergyFromTendersOnly(
    Map<TrainTypes::PowerType, double> EC, double &timeStep)
{
    for (auto &locoType : EC.get_keys())
    {
        TrainTypes::CarType tenderType =
            TrainTypes::powerToCarMap.at(locoType);
        for (auto &tender :
             this->ActiveCarsTypes[tenderType])
        {
            EC[locoType] +=
                tender->getMaxProvidedEnergy(timeStep);
        }
    }
    return EC;
}

std::pair<double, Map<TrainTypes::PowerType, double>>
Train::getMaxProvidedEnergy(double &timeStep)
{
    // define a var to hold the result
    Map<TrainTypes::PowerType, double> locoMaxEC =
        this->getMaxProvidedEnergyFromLocomotivesOnly(
            timeStep);

    if (locoMaxEC.get_keys().size() == 0)
    {
        return std::make_pair(
            0.0, Map<TrainTypes::PowerType, double>());
    }

    auto out = this->getMaxProvidedEnergyFromTendersOnly(
        locoMaxEC, timeStep);

    // sum all values in the map
    double res = std::accumulate(
        out.begin(), out.end(), 0.0,
        [](const double prev_sum,
           const std::pair<TrainTypes::PowerType, double>
               &entry) { return prev_sum + entry.second; });

    return std::make_pair(res, out);
}

Map<TrainTypes::CarType, double>
Train::canProvideEnergyFromLocomotivesOnly(double &EC,
                                           double &timeStep)
{
    // define a var to hold the result
    ...
    double dividedEC =
        EC / ((double)this->ActiveLocos.size());
    // check all active locomotives, if any cannot provide,
    // return not the rest of the energy required to get
    // later from tenders
    for (auto &loco : this->ActiveLocos)
    {
        double locoRest = loco->canProvideEnergy(
            dividedEC,
            timeStep); // this is only checked for electric
                       // trains only
        TrainTypes::CarType tenderType =
            TrainTypes::powerToCarMap.at(loco->powerType);

        // add value to locoRestEC
        if (locoRestEC.is_key(tenderType))
        {
            locoRestEC[tenderType] = locoRest;
        }
        else
        {
            locoRestEC[tenderType] += locoRest;
...
bool Train::canProvideEnergyFromTendersOnly(
    Map<TrainTypes::CarType, double> &EC, double &timeStep)
{
    bool result = true;

    for (auto &tenderT : EC.get_keys())
    {
        double dividedEC_tenders =
            EC[tenderT]
            / this->ActiveCarsTypes[tenderT].size();
        for (auto &tender : this->ActiveCarsTypes[tenderT])
        {
            result = result
                     && tender->canProvideEnergy(
                         dividedEC_tenders, timeStep);
            //            result *=
            //            tender->canProvideEnergy(dividedEC_tenders,
            //            timeStep);
        }
    }
...
bool Train::canProvideEnergy(double &EC, double &timeStep)
{
    // define a var to hold the result
    Map<TrainTypes::CarType, double> locoRestEC =
        this->canProvideEnergyFromLocomotivesOnly(EC,
                                                  timeStep);

    if (locoRestEC.get_keys().size() == 0)
    {
        return false;
    }

    if (locoRestEC.get_values().sum() == 0.0)
    {                // if no tenders
        return true; // return true if all energy can be
                     // provided and false o.w
    }
    return this->canProvideEnergyFromTendersOnly(locoRestEC,
                                                 timeStep);
}
...
QJsonObject Train::getCurrentStateAsJson()
{
    ...
    jsonState["trainUserID"] =
        QString::fromStdString(trainUserID);
    jsonState["totalLength"] = totalLength;
    jsonState["totalMass"] =
        totalMass / 1000.0; // Convert kg to tons
    ...
    jsonState["totalCarbonDioxideEmitted"] =
        totalCarbonDioxideEmitted / 1000.00; // kg;
    jsonState["cumulativeDelayTimeStat"] = cumDelayTimeStat;
    jsonState["cumulativeMaxDelayTimeStat"] =
        cumMaxDelayTimeStat;
    ...
    for (const auto &kvp : consumedTank)
    {
        fuel.insert(QString::fromStdString(kvp.first),
                    kvp.second);
    }
    ...
    jsonState["currentTractiveForce"] =
        currentTractiveForce;
    jsonState["currentResistanceForces"] =
    ...
    qDebug() << "Train::getCurrentStateAsJson()"
             << jsonState;
    return jsonState;
}
...
{
    this->lookAheadCounterToUpdate =
        mem_lookAheadCounterToUpdate;
...
{
    ostr << "Freight Train:: ID: " << train.trainUserID
         << ", length (m): " << train.totalLength;
    ostr << ", total weight (t): "
         << train.totalMass / 1000; // convert to tons
    ostr << ", # locomotives: " << train.nlocs
         << ", # cars: " << train.nCars;
    ostr << ", travelled distance (m):"
         << train.travelledDistance << std::endl;
    return ostr;
};
