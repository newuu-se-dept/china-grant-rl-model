...
// This function readTrainsFile takes a string fileName as
// the file to read trains from. The function returns the
// trains as objects
Vector<Map<std::string, std::any>>
TrainsList::readTrainsFile(const std::string &fileName)
{
    ...
    // if the file has trains continue, else stop and throw
    // error
    if (lines.size() == 0)
    {
        throw std::runtime_error(
            "Error: "
            + std::to_string(
                static_cast<int>(Error::trainsFileEmpty))
    ...
    try
    {
        for (int i = 2; i < lines.size(); ++i)
        {
            ...
            // declare the locomotives, cars vectors for
            // each train
            // std::vector<std::vector<std::string>>
            // trainsCharacteristics;
            ...
            if (!lv.empty())
            {
                ...
                std::vector<std::string> p =
                    Utils::split(locomotivesDef, ';', true);
                for (const auto &p_ : p)
                {
                    std::vector<std::string> loc =
                        Utils::split(p_, ',', true);
                    if (loc.size()
                        != locomotiveFieldsOrder.size())
                    {
                        throw std::runtime_error(
                            "Wrong Trains File "
                            "Strucuture!");
                    }
                    // if we have multiple locomotive types,
                    // add them all
                    Map<std::string, std::string>
                        locomotiveRecord;
                    for (const auto &key :
                         locomotiveFieldsOrder)
                    {
                        int keyIndex =
                            locomotiveFieldsOrder.index(
                                key);
                        locomotiveRecord[key] =
                            loc[keyIndex];
                    }
                    locomotiveRecords.push_back(
                        locomotiveRecord);
                }
                ...
                std::vector<std::string> cp =
                    Utils::split(carsDef, ';', true);
                for (const auto &cp_ : cp)
                {
                    std::vector<std::string> c =
                        Utils::split(cp_, ',', true);
            ...
            Vector<int> trainPath =
                Utils::splitStringToIntVector(lv[1]);
...
std::shared_ptr<Train> TrainsList::generateTrain(
    Map<std::string, std::any> &trainRecord,
    bool                        resetTrainID)
{
    if (resetTrainID)
    {
        trainID.store(
            0,
            std::memory_order_release); // Ensure visibility
                                        // across threads
    }
    ...
    try
    {
        ...
        // get loco values
        auto locoRecords = std::any_cast<
            Vector<Map<std::string, std::string>>>(
            trainRecord["Locomotives"]);
        for (auto &locoRecord : locoRecords)
        {
            for (int i = 0;
                 i < std::stoi(locoRecord["Count"]); i++)
            {
                Locomotive loco = Locomotive(
                    stod(locoRecord["Power"]), // Power
                    stod(locoRecord
                             ["TransmissionEff"]), // trans
                                                   // eff
                    stod(locoRecord["Length"]),    // length
                    stod(
                        locoRecord["AirDragCoeff"]), // drag
                    stod(locoRecord["FrontalArea"]), // area
                    stod(locoRecord
                             ["GrossWeight"]),     // weight
                    stoi(locoRecord["NoOfAxles"]), // axles
                    stoi(locoRecord["Type"])); // power type
                locomotives.push_back(
                    std::make_shared<Locomotive>(loco));
            }
        }

        // get car values
        auto carRecords = std::any_cast<
            Vector<Map<std::string, std::string>>>(
            trainRecord["Cars"]);
        for (auto &carRecord : carRecords)
        {
            for (int i = 0;
                 i < std::stoi(carRecord["Count"]); i++)
            {
                Car car = Car(
                    std::stod(
                        carRecord["Length"]), // length
                    std::stod(
                        carRecord["AirDragCoeff"]), // drag
                    std::stod(
                        carRecord["FrontalArea"]), // area
                    std::stod(
                        carRecord["TareWeight"]), // empty
                                                  // weight
                    std::stod(
                        carRecord["GrossWeight"]), // full
                                                   // weight
                    std::stoi(
                        carRecord["NoOfAxles"]),   // axles
    ...
    bool                                resetTrainIDs)
{
    ...
    {
        auto &trainRecord = trainRecords[i];
        bool  reset       = false;
        if (i == 0 && resetTrainIDs)
        {
            reset = true;
        }
        trains.push_back(
            std::move(generateTrain(trainRecord, reset)));
    }
    return trains;
}

Vector<std::shared_ptr<Train>>
TrainsList::ReadAndGenerateTrains(
    ...
    const std::string                 &fileName)
{
    ...
    {
        auto locos = std::any_cast<
            Vector<Map<std::string, std::string>>>(
            record["Locomotives"]);
        std::size_t locosSize = locos.size();

        auto cars = std::any_cast<
            Vector<Map<std::string, std::string>>>(
            record["Cars"]);
        std::size_t carsSize = cars.size();

        lines << std::any_cast<std::string>(
            record["UserID"])
              << "\t"; // ID
        lines << std::any_cast<Vector<int>>(
                     record["TrainPathOnNodeIDs"])
                     .toNotFormattedString()
              << "\t"; // Path
        lines << std::any_cast<double>(record["LoadTime"])
              << "\t"; // time
        lines << std::any_cast<double>(
            record["FrictionCoef"])
              << "\t"; // friction coef
        ...
        {
            auto &car = cars[i];
            lines << car.valuesToString(
                carFieldsOrder, Vector<std::string>(), ",")
                  << (i != (carsSize - 1) ? ";" : "");
        }
    }
...
Vector<Map<std::string, std::any>>
TrainsList::readTrainsFromJSON(
    const QJsonObject &jsonObject)
{
    auto getStringOfValue =
        [](const QJsonValue &value) -> std::string {
        if (value.isString())
        {
        ...
        {
            return QString::number(value.toDouble(), 'f',
                                   15)
                .toStdString();
        }
    ...
    for (const QJsonValue &trainValue : trainsArray)
    {
        ...
        Vector<Map<std::string, std::string>>
                   locomotiveRecords;
        QJsonArray locomotivesArray =
            trainObject["Locomotives"].toArray();
        for (const QJsonValue &locoValue : locomotivesArray)
        {
            ...
            {
                locomotiveRecord[key] = getStringOfValue(
                    locoObject[QString::fromStdString(
                        key)]);
            }
        ...
        for (const QJsonValue &carValue : carsArray)
        {
            ...
            for (const auto &key : carFieldsOrder)
            {
                carRecord[key] = getStringOfValue(
                    carObject[QString::fromStdString(key)]);
            }
            carRecords.push_back(carRecord);
        }
        ...
        QJsonArray pathArray =
            trainObject["TrainPathOnNodeIDs"].toArray();
        Vector<int> trainPath;
        for (const QJsonValue &pathValue : pathArray)
        {
            ...
            if (pathValue.isString())
            {
                ...
                {
                    qWarning()
                        << "Invalid node ID format in path:"
                        << pathValue.toString();
                }
            }
        ...
        Map<std::string, std::any> trainRecord = {
            {"UserID", trainObject["UserID"]
                           .toString()
                           .toStdString()},
            {"TrainPathOnNodeIDs", trainPath},
            {"LoadTime",
             trainObject["LoadTime"].toDouble()},
            {"FrictionCoef",
             trainObject["FrictionCoef"].toDouble()},
            {"Locomotives", locomotiveRecords},
            {"Cars", carRecords},
...
{
    auto trainRecords =
        TrainsList::readTrainsFromJSON(jsonObject);
